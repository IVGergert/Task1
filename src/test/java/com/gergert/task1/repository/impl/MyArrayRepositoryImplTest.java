...
class MyArrayRepositoryImplTest {
    ...
    void add() throws CustomException {
        MyArray myArray = new MyArray(1, new int[]{1, 2, 3});

        repository.add(myArray);

        Optional<MyArray> result = repository.findArrayById(1);
        assertTrue(result.isPresent());
        assertEquals(myArray, result.get());
    ...
    void addDuplicate() throws CustomException {
        MyArray array1 = new MyArray(1, new int[]{1});
        MyArray array2 = new MyArray(1, new int[]{2});
    ...
    void remove() throws CustomException {
        ...
        repository.remove(myArray);

        assertFalse(repository.findArrayById(1).isPresent());

        assertNull(ArrayWarehouseImpl.getInstance().get(1));

        myArray.setElement(0, 999);
        assertNull(ArrayWarehouseImpl.getInstance().get(1));
    }

    @Test
    void removeNonExistent() {
        MyArray myArray = new MyArray(99, new int[]{1});
        assertThrows(CustomException.class, () -> repository.remove(myArray));
    }

    @Test
    void findArrayById() throws CustomException {
        MyArray arr1 = new MyArray(10, new int[]{1});
        MyArray arr2 = new MyArray(20, new int[]{2});
        repository.add(arr1);
        repository.add(arr2);

        Optional<MyArray> found = repository.findArrayById(10);
        assertTrue(found.isPresent());
        assertEquals(10, found.get().getId());

        Optional<MyArray> notFound = repository.findArrayById(99);
        assertFalse(notFound.isPresent());
    }

    @Test
    void query() throws CustomException {
        repository.add(new MyArray(1, new int[]{1, 2, 3}));
        repository.add(new MyArray(2, new int[]{10, 10}));
        repository.add(new MyArray(3, new int[]{0, 0}));
        ...
        List<MyArray> result = repository.query(sumMoreThan10);
    ...
    void sort() throws CustomException {
        MyArray arr1 = new MyArray(3, new int[]{1,6,2});
        MyArray arr2 = new MyArray(1, new int[]{1,9,2});
        MyArray arr3 = new MyArray(2, new int[]{2,9,34});

        repository.add(arr1);
        repository.add(arr2);
        repository.add(arr3);

        Comparator<MyArray> idComparator = Comparator.comparingInt(MyArray::getId);

        List<MyArray> sorted = repository.sort(idComparator);
    ...
    void testIntegrationUpdate() throws CustomException {
        int id = 5;
        MyArray array = new MyArray(id, new int[]{10, 20});
